...
subroutine vertvel(ia,iam,ja,ka)
  ...
  kloop: do k=1,ka
     ...
     do n=n1,n2
        ...
        ! Note that the horizontal velocities have already been multiplied by ff
        ! The time dependent layer thickness must hiowever be multlipilied by ff
        wflux(k,n) = wflux(k-1,n) -  &
             (  uflux(ia,ja,k,n) - uflux(iam, ja,   k, n)   & 
              + vflux(ia,ja,k,n) - vflux(ia,  ja-1, k, n)   & 
              - ff*(dzt(ia,ja,k,n2)-dzt(ia,ja,k,n1))*dxdy(ia,ja)/tseas )
    enddo
#else
     ...
     wflux(ia,ja,k,nsm)=wflux(ia,ja,k-1,nsm) - ( uu - um + vv - vm )
#else
    do n=n1,n2
     wflux(k,n) = wflux(k-1,n) - &
