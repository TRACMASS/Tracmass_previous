...
CONTAINS
  ...
  subroutine turbuflux(ia,ja,ka,dt)
    ...
    !  qran=2.*qran-1. ! === Max. amplitude of turbulence with random numbers
    !                        between -1 and 1
    ...
    ! (this makes it unstable!) multiply the time interpolated velocities by 
    ! random numbers 4 different random velocities at the 4 walls
    !   upr(:,2)=uv(:)*rand(:) 
    ! or same random velocities at the eastern and western as well as 
    ...
    ! upr(:,1)=upr(:,2) ! impose same velocities for t-1 and t 
    !    (this makes it unstable! but why? K.Döös)
    ...
    do n=1,2
       ...
       !    upr(5,n) = w(ka-1) - ff * 
       !       ( upr(1,n) - upr(2,n) + upr(3,n) - upr(4,n) )
       !    upr(6,n) = 0.d0
       ! = Detta gör att man bara justerar vertikala hastigheten på
       !    ovansidan av boxen ! u2
